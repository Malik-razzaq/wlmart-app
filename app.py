...
# ════════════════════════════
    ...
    st.info(
        ...
        "Top solutions on similar datasets achieve approximately 2-3% MAPE "
    ...
    benchmark = pd.DataFrame({
        "Model"   : [
            ...
            "Top solutions (similar datasets)",
        ],
        "MAPE"    : ["~18%","9.17%","4.43%","~2-3%"],
        "Notes"   : [
            "Baseline to beat",
            "8 raw features only",
            "35 engineered features — near ceiling",
            "Uses promotion + markdown data (not available here)"
        ]
    })
    st.dataframe(benchmark, use_container_width=True)
    st.warning(
        "Gap to top solutions is due to missing promotion, "
