...
if page == "Dashboard":
    ...
    fig_trend = px.line(
        ...
        title=f"Store {store_filter} Weekly Sales Over Time"
    )
...
# ════════════════════════════
    ...
    if "Bias_Dir" in results.columns:
...
# ════════════════════════════
    ...
    st.info(
        "This project uses the Walmart Store Sales dataset (Kaggle). "
        "SOTA comparison is based on best known public solutions for this dataset. "
        "Exact SOTA for this specific dataset is not officially published — "
        "top Kaggle solutions on similar datasets achieve approximately 2-3% MAPE "
        "using additional promotion and markdown data not available here."
    )
    ...
    benchmark = pd.DataFrame({
        "Model"  : [
            ...
            "Top Kaggle solutions (similar datasets)",
        ],
        "MAPE"   : ["~18%", "9.17%", "4.43%", "~2-3%"],
        "Features": [
            "None — just lag_1",
            "8 raw features",
        ...
        "Notes"  : [
            "Baseline to beat",
            "Starting point",
            "Near performance ceiling for this dataset",
            "Data not available in public dataset"
        ]
    })
    st.dataframe(benchmark, use_container_width=True)
    st.warning(
        "Gap to top solutions (~1.5pp) is due to missing promotion, "
        "markdown and store-type data. With available features, "
        "4.43% MAPE is near the performance ceiling for this dataset."
    )
...
# ════════════════════════════
    ...
    st.caption("Enter store details to get an instant weekly sales forecast")
    st.divider()
    ...
    if st.button("Generate Forecast", type="primary", use_container_width=True):
        ...
        try:
            ...
            st.write("Expected features:", list(features) if features else "Not loaded")
