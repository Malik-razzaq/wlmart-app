...
if page == "📊 Dashboard":
    ...
    st.subheader(f"Store {store_filter} Weekly Sales Trend")
    fig_trend = px.line(
        filtered_df, x="Date", y="Weekly_Sales",
        title=f"Store {store_filter} Sales Over Time"
    )
...
# ════════════════════════════
    ...
    st.title("🗺️ Error Heatmap — Store x Month")
    st.divider()
...
# ════════════════════════════
    ...
    benchmark = pd.DataFrame({
        ...
        "MAPE"    : ["~18%","9.17%","4.43%","~2-3%","~1-2%"],
        "Notes"   : [
            "Last week sales as prediction",
            "Raw 8 features only",
            "35 engineered features",
            "With promotion + markdown data",
            "Full feature dataset"
        ]
    })
    st.dataframe(benchmark, use_container_width=True)
    st.info(
        "Gap to SOTA (~2%) is due to missing promotion, "
        "markdown and store-type data not available in this dataset."
    )
